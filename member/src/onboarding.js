...
export function handleMemberCommand(whatsapp, args) {
  ...
  if (command === "setuju") {
    ...
    if (existing) {
      setSession(whatsapp, { memberId: existing.member_id, step: "NAME" });
      return { reply: "✅ Data Member sudah terdaftar. Kita lanjutkan verifikasi dari data yang masih diperlukan." };
    }
  ...
  if (command === "nama") {
    ...
    saveMember(updated);
    setSession(whatsapp, { memberId: updated.member_id, step: "EMAIL" });
  ...
  if (command === "email") {
    const gmail = args[1] || "";
    if (!/^[^\\s@]+@[^\\s@]+\\.[^\\s@]+$/.test(gmail)) {
      return { reply: "Format email belum valid." };
    }
    const updated = { ...current, gmail };
    saveMember(updated);
    setSession(whatsapp, { memberId: updated.member_id, step: "SOCIAL" });
  ...
  if (command === "po") {
    ...
    if (!value || !["bussid", "ets2"].includes(game)) {
      return { reply: "Format: /member po bussid Nama PO atau /member po ets2 Nama PO" };
    }
    const patch = game === "bussid" ? { po_bussid: value } : { po_ets2: value };
  ...
  if (command === "upload") {
    return {
      reply: [
        "Format upload: kirim foto dengan caption:",
        "/member upload SOCIAL_SCREENSHOT",
        "/member upload MOD_OWNERSHIP",
        "/member upload KTP",
        "/member upload KK",
        "/member upload SIM",
        "/member upload KARTU_PELAJAR",
        "/member upload FACE"
      ].join("\n")
    };
  }
  ...
  if (command === "status") {
    return { reply: "Status Member: " + current.status + " | ID: " + current.member_id };
  }

  return { reply: "Perintah member: /member daftar | setuju | nama | email | po | upload | status" };
}
